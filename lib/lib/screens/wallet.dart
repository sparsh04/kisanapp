...
class _MyWalletState extends State<MyWallet> {
  ...
  onsuccess() async {
    ...
    FirebaseFirestore.instance.collection("Payments").doc(v).set(userInfoMap);
  ...
  Future<double> getTotal() async {
    ...
    await FirebaseFirestore.instance
      ...
      List<DocumentSnapshot> ls = value.docs;
      ls.forEach((element) {
        num pri = element['amount'];
        purchase += (pri);
      });
    });
    double to = 0.00;
    to = recharge - purchase;

    await FirebaseFirestore.instance
        .collection('users')
        .doc(FirebaseAuth.instance.currentUser!.phoneNumber)
        .update({"wallettotal": to});
